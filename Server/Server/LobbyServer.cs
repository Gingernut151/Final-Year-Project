...
namespace FYP_Server
{
    class LobbyServer
    {
        ...
        public void InitLobbyConnection()
        {
            ...
            tcp_Config.address =  "127.0.0.1"; //"0.0.0.0"; //
        ...
        public void UpdateLobby()
        {
            while (true)
            {
                ...
                if (StartMatch() == true)
                {
                    LobbyReadyPacket packet = new LobbyReadyPacket("Server", true, 0, "0", "Default", Protocol.UDP, PacketSize.LARGE);
            ...
            if (packetList.Count() > 0)
            {
                try
                {
                    ...
                    if (packetList[0].type == PacketType.LOBBYREADY)
                    {
                        ...
                        string testName = ((LobbyReadyPacket)packetList[0]).testName;
                        string serverTickrate = ((LobbyReadyPacket)packetList[0]).serverTickrate;
                        Protocol protocol = ((LobbyReadyPacket)packetList[0]).protocol;
                        PacketSize packetSize = ((LobbyReadyPacket)packetList[0]).packetSize;

                        if (packetList[0].sender.Equals(clientsConnected[0].username))
                        {
                            MatchServer.protocol = protocol;
                            MatchServer.packetSize = packetSize;
                            MatchServer.sendRate = Int32.Parse(serverTickrate);
                            TestResults._testName = testName;
                        }
        ...
        {
            timer.Stop();
            deltaTime += (timer.Duration() / 1000.0f);
            timer.Reset();
            timer.Start();
